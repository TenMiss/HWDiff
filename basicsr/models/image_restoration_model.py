...
class ImageCleanModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        ################ add load rough estimation models
        self.net_g_rough = define_network(deepcopy(opt['network_g_rough']))
        self.net_g_rough = self.model_to_device(self.net_g_rough)
        # self.print_network(self.net_g_rough)
        load_path_rough = self.opt['path'].get('pretrain_network_g_rough_fix', None)   
        if load_path_rough is not None:
            self.load_network(self.net_g_rough, load_path_rough,
                              self.opt['path'].get('strict_load_g', True), param_key=self.opt['path'].get('param_key', 'params'))
        for (name, param) in self.net_g_rough.named_parameters():
    ...
    def init_training_settings(self):
        ...
        #     pixel_type3 = train_opt['pixel_opt3'].pop('type')
        #     cri_pix_cls = getattr(loss_module, pixel_type3)
        #     self.cri_pix3 = cri_pix_cls(**train_opt['pixel_opt3']).to(
        #         self.device)
    ...
    def feed_train_data(self, data):
        ...
        self.rough_preds = self.net_g_rough(self.lq)
        if 'gt' in data:
    ...
    #     self.rough_preds = self.data_transform(self.rough_preds)
    ...
    #     ##### noise
    #     preds = self.net_g(torch.cat([self.lq, x], dim=1), self.rough_preds, t.float())
    ...
    #         l_pix += self.cri_pix3(self.rough_preds, self.gt)
    ...
    def optimize_parameters(self, current_iter):
        ...
        for i, j in zip(reversed(seq), reversed(seq_next)):
            ...
            xt = xs[-1]
            ### change self.lq
            et = self.net_g(torch.cat([self.lq, xt], dim=1), self.rough_preds, t)
        ...
        # pixel loss
        l_pix = 0.84 * (1 - self.cri_pix1(xt_next, self.gt)) + self.cri_pix2(xt_next, self.gt) + self.cri_pix3(self.rough_preds, self.gt) # ssim+l1+dwt
    ...
    def generalized_steps(self, img, model, eta=0.):
        ...
        img_preds = self.net_g_rough(img)
        x_hf = self.data_transform(img_preds)
        ...
        # print(x_cond.shape, x.shape, x_hf.shape)
        for i, j in zip(reversed(seq), reversed(seq_next)):
            ...
            at_next = self.compute_alpha(self.betas, next_t.long())
            xt = xs[-1]
            et = model(torch.cat([x_cond, xt], dim=1), x_hf, t)
            x0_t = (xt - et * (1 - at).sqrt()) / at.sqrt()
